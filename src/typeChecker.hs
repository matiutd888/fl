...
checkForBool t = case t of
    A.Bool _ -> return ()
    t -> throwError (errorMessage1 t $ A.Bool $ A.hasPosition t)


errorMessage1 :: Type -> Type -> String
errorMessage1 received expected = unexpectedTypeMessage received  ++ ", " ++ (expectedTypeMessage  expected)

unexpectedTypeMessage :: Type -> String 
unexpectedTypeMessage t = "Unexpected type " ++ printTree t ++ " on position " ++ (show $ A.hasPosition t)
